...
class BeardCounterGroundedSAM:
    ...
    def update_display(self):
        ...
        info = f"Total: {total} | Remove: {active} ({self.removal_percentage}%) | Mode: {self.selection_mode}"
        cv2.rectangle(self.display_image, (5, 10), (500, 35), (0, 0, 0), -1)
    ...
    def save_inpainted(self, filename: str = "beard_inpainted.png"):
        ...
        print(f"Inpainting結果を保存: {filename}")

    def run(self):
        window_name = 'Beard Counter (Grounded SAM)'
        ...
        print("=" * 70)
        print("髭検出アプリケーション - Grounded SAM版")
        print("=" * 70)
        print("操作方法:")
        print("  [検出モード]")
        ...
        print("  [表示]")
        print("    'p': Inpaintingプレビュー / 'n': 新しいランダムシード")
        print("  [保存]")
        ...
        while True:
            ...
            status = f"Mode: {mode_names.get(self.mode, self.mode)} | Prompt: {self.text_prompt[:30]}"
