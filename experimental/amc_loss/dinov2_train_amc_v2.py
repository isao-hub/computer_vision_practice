...
class AMCLoss(nn.Module):
    ...
    推論時のコサイン類似度ヒートマップの「キレ」を良くするための学習ロジック
    """
    ...
    def forward(self, embeddings, labels):
        """
        Args:
            embeddings: [batch_size, embedding_dim] (L2正規化済み)
            labels: [batch_size]
        """
        device = embeddings.device
        # 1. 類似度行列の計算 (Cosine Similarity)
        # embeddings は既に F.normalize されている前提
        ...
        # 3. AMCの核心：ポジティブ（同クラス）とネガティブ（別クラス）で処理を分ける
        # 同クラスのペアにはマージンを適用せず、別クラスとの境界を押し広げる
        # 角度空間でのマージンを近似的に扱うため、別クラスのスコアにマージンを加算して「近く」見せ、
        # それを突き放すように学習させる
        
        # 類似度スコアを温度パラメータでスケール
        logits = cosine_sim / self.temperature

        # ネガティブ（別クラス）のスコアにマージンを加算（より厳しく引き離す）
        # mask=0の場所（別クラス）に対してのみ margin/temp を足す
        negative_margin_logits = logits + ((1.0 - mask) * (self.margin / self.temperature))

        # 4. Log-Softmaxによる損失計算
        # 各サンプルについて、ポジティブペアの尤度を最大化する
        exp_logits = torch.exp(negative_margin_logits) * logits_mask
        log_prob = logits - torch.log(exp_logits.sum(dim=1, keepdim=True) + 1e-6)

        # ポジティブペアが存在するサンプルのみ平均をとる
        mask_sum = mask.sum(dim=1)
        ...
        mean_log_prob_pos = (mask * log_prob).sum(dim=1)[valid_indices] / mask_sum[valid_indices]
        loss = -mean_log_prob_pos.mean()
...
def main():
    ...
    parser.add_argument('--temperature', type=float, default=0.07, help='Temperature for contrastive loss (default: 0.07, lower=harder)')
    ...
    contrastive_criterion = AMCLoss(margin=0.1, temperature=args.temperature)
