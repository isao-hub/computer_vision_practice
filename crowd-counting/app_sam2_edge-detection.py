"""SAM2 (Hiera Encoder) based hair detection and classification.

1. 毛質別のカウント:
   SAM2で検出した髭を太さ（幅）に基づいて4種類に分類:
   - 剛毛: 赤
   - 硬毛: オレンジ
   - 中間毛: 緑
   - 軟毛: 紫
...
SAM2の特徴:
   - Hiera (Hierarchical Vision Transformer) encoder による階層的マルチスケール特徴抽出
   - Mask-to-Mask (M2M) refinement によるマスク精度向上
   - sam2.1_hiera_tiny (軽量・高速) / sam2.1_hiera_large (高精度) を選択可能

Usage:
    python app_sam2_edge-detection.py
...
from utils.black_white_hair_detector_sam2 import (
    BlackWhiteHairDetectorSAM2,
    SAM2_MODEL_CONFIGS,
)


class EdgeDetectionAppV3SAM2:
    """Gradio application v3 (SAM2) for hair thickness classification and black/white count."""

    def __init__(self):
        self._detector = BlackWhiteHairDetectorSAM2()
    ...
    def detect_and_classify(
        ...
        Returns:
            ...
            all_mask: All SAM2 masks (before filtering)
        ...
        sam2_kwargs = dict(
            ...
            use_m2m=sam2_use_m2m,
            multimask_output=sam2_multimask_output,
        )

        # Run SAM2 detection
        t_sam2_start = time.perf_counter()
        if selection_mode == "freeform" and freeform_mask is not None:
            detections, all_masks, stats = self._detector.detect_with_class_and_mask(
                image, freeform_mask, hair_class, params, **sam2_kwargs,
            )
        else:
            detections, all_masks, stats = self._detector.detect_with_class(
                image, rect, hair_class, params, **sam2_kwargs,
            )

        t_sam2_end = time.perf_counter()
        ...
        t_pipeline_preprocess = t_preprocess_end - t_preprocess_start
        t_pipeline_sam2 = t_sam2_end - t_sam2_start
        t_pipeline_mask_vis = t_vis_end - t_vis_start

        # Retrieve detector internal timing if available
        detector_timing = stats.get('timing', {})

        print("\n╔═══════════════════════════════════════════════════╗")
        print("║          Full Pipeline Timing Summary             ║")
        print("╠══════════════════════════════╦══════════╦═════════╣")
        print("║ Step                         ║ Time (s) ║    %    ║")
        print("╠══════════════════════════════╬══════════╬═════════╣")

        timing_rows = [
            ("1. Preprocessing (ROI etc.)", t_pipeline_preprocess),
            ("2. SAM2 Detection (total)", t_pipeline_sam2),
        ]
        if detector_timing:
            timing_rows.append(("   2a. AMG Generate", detector_timing.get('sam2_amg_generate', 0)))
            # SAM2 internal breakdown
            if detector_timing.get('image_encoder', 0) > 0 or detector_timing.get('prompt_encoder', 0) > 0:
                timing_rows.extend([
                    ("     - Image Encoder (Hiera)", detector_timing.get('image_encoder', 0)),
                    ("     - Prompt Encoder", detector_timing.get('prompt_encoder', 0)),
                    ("     - Mask Decoder", detector_timing.get('mask_decoder', 0)),
                    ("     - AMG Other (NMS etc.)", detector_timing.get('amg_other', 0)),
                ])
            timing_rows.extend([
                ("   2b. Mask Store", detector_timing.get('mask_store', 0)),
                ("   2c. Mask Filter", detector_timing.get('mask_filter', 0)),
                ("   2d. Duplicate Removal", detector_timing.get('duplicate_removal', 0)),
            ])
        timing_rows.append(("3. Mask Visualization", t_pipeline_mask_vis))
        timing_rows.append(("4. Post-process (below)", None))  # placeholder

        t_so_far = t_pipeline_preprocess + t_pipeline_sam2 + t_pipeline_mask_vis
        # Store start time for post-processing measurement
        self._t_postprocess_start = time.perf_counter()
        self._t_total_start = t_total_start
        ...
        if detection_mode == "色別のカウント":
            return self._process_bw_count_mode(
                ...
                sam2_model_name, sam2_pred_iou_thresh,
                sam2_stability_score_thresh, sam2_use_m2m,
            )
        else:
    ...
    def _process_thickness_mode(
        ...
        model_label = SAM2_MODEL_CONFIGS[model_name]["label"]
        ...
        status_lines = [
            ...
            "",
            "━━━ SAM2 パラメータ ━━━",
            f"points_per_side={sam_points_per_side}, "
            f"pred_iou_thresh={pred_iou_thresh:.2f}, "
            f"stability_score_thresh={stability_score_thresh:.2f}",
            f"use_m2m={use_m2m}",
            "",
            "━━━ フィルタ統計 ━━━",
            f"SAM2 masks: {stats['total']}",
    ...
    def _process_bw_count_mode(
        ...
        status_lines.extend([
            ...
            "",
            "━━━ SAM2 パラメータ ━━━",
            f"points_per_side={sam_points_per_side}{tiles_info}, "
            f"pred_iou_thresh={pred_iou_thresh:.2f}, "
            f"stability_score_thresh={stability_score_thresh:.2f}",
            f"use_m2m={use_m2m}",
            "",
            "━━━ フィルタ統計 ━━━",
            f"Total masks: {stats['total']}",
            f"Filtered: area_small={stats['filtered_area_small']}, "
            f"area_large={stats['filtered_area_large']}, "
            f"aspect={stats['filtered_aspect']}, "
            f"brightness={stats['filtered_brightness']}",
    ...
    def _print_pipeline_timing(self):
        """Print the full pipeline timing summary to terminal."""
        t_postprocess = time.perf_counter() - self._t_postprocess_start
        t_total = time.perf_counter() - self._t_total_start

        timing_rows = self._timing_rows
        # Replace the placeholder with actual post-process time
        timing_rows = [(label, t) for label, t in timing_rows if t is not None]
        timing_rows.append(("4. Post-process & classify", t_postprocess))
...
def create_app():
    ...
    app = EdgeDetectionAppV3SAM2()

    # Build model choices from SAM2_MODEL_CONFIGS
    model_choices = [
        (config["label"], name)
        for name, config in SAM2_MODEL_CONFIGS.items()
    ]

    with gr.Blocks(
        title="Hair Detection v3 (SAM2) - Thickness & BW Count",
        theme=gr.themes.Soft()
    ) as demo:
        gr.Markdown("""
        # 髭検出・分類 (SAM2 / Hiera Encoder)

        SAM2の**Hiera (Hierarchical Vision Transformer)** encoderを使用した髭検出。
        階層的マルチスケール特徴抽出とMask-to-Mask refinementにより高精度なセグメンテーションを実現。
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                with gr.Accordion("SAM2 Settings", open=True):
                    sam2_model_name = gr.Dropdown(
                        choices=model_choices,
                        value="sam2.1_hiera_tiny",
                        label="SAM2 モデル",
                        info="Tiny: 軽量・高速 / Large: 高精度・低速",
                    )
                    sam_points_per_side = gr.Slider(
                        minimum=16, maximum=128, value=64, step=8,
                        label="Points Per Side",
                        info="サンプリング密度 (64-96 推奨)"
                    )
                    sam2_pred_iou_thresh = gr.Slider(
                        minimum=0.3, maximum=1.0, value=0.7, step=0.05,
                        label="Pred IoU Threshold",
                        info="IoU予測の閾値。低いほど多くのマスクを保持"
                    )
                    sam2_stability_score_thresh = gr.Slider(
                        minimum=0.3, maximum=1.0, value=0.8, step=0.05,
                        label="Stability Score Threshold",
                        info="安定性スコアの閾値。低いほど多くのマスクを保持"
                    )
                    sam2_use_m2m = gr.Checkbox(
                        value=True,
                        label="Mask-to-Mask Refinement (use_m2m)",
                        info="SAM2固有: マスクを再入力して精度を向上"
                    )
                    sam2_multimask_output = gr.Checkbox(
                        value=True,
                        label="Multi-mask Output",
                        info="複数マスク候補を生成して最良を選択"
                    )
            ...
            with gr.Column(scale=1):
                ...
                status_text = gr.Textbox(label="ステータス", lines=16)
        ...
        **Pred IoU Threshold**: SAM2が予測するマスク品質スコア(IoU)の閾値。
        低くすると多くのマスクが残り、高くすると高品質なマスクのみ残る。

        **Stability Score Threshold**: マスクの安定性スコア。
        同じ領域で少し閾値を変えてもマスクが安定しているかを測定。

        **Mask-to-Mask (M2M) Refinement**: SAM2固有の機能。
        初回生成したマスクをモデルに再入力し、マスク境界を精緻化する。
