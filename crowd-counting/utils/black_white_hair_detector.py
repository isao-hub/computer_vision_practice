...
def patch_torch_for_mps():
    ...
    Only applied when MPS backend is available (macOS).
    Skipped on CUDA to avoid torchvision JIT compilation conflicts.
    """
    if not TORCH_AVAILABLE:
        return
    ...
    # Skip patch on non-MPS environments (CUDA, CPU-only)
    if not (hasattr(torch.backends, 'mps') and torch.backends.mps.is_available()):
        print("torch.as_tensor patch skipped (MPS not available)")
        return
